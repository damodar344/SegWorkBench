...
class MaskEditorTab(ttk.Frame):
    """
    Simple binary mask editor:
      - Load image + optional mask
      - Brush / Eraser
      - Overlay (red fill + green edge)
      - Zoom + Fit-to-window
      - Undo
    """
    def __init__(self, master):
        ...
        # --- State ---
        self.img_pil = None              # full-res RGB
        self.mask_arr = None             # full-res uint8 0/255

        # display assets (scaled)
        self.disp_img = None             # scaled RGB PIL
        self.disp_mask = None            # scaled uint8 0/255 np
        ...
        self.zoom = tk.DoubleVar(value=1.0)     # additional zoom on top of fit-to-window scale
        self._fit_scale = 1.0                   # computed from canvas size

        self.status = tk.StringVar(value="Idle")
        self.undo_stack = []
        self.max_undo = 20

        # canvas offsets (for centering)
        ...
        ttk.Scale(tools, from_=1, to=120, orient="horizontal", variable=self.brush_size).pack(side="left", fill="x", expand=True)

        ttk.Label(tools, text="Opacity:").pack(side="left", padx=(15, 5))
        ttk.Scale(tools, from_=0.05, to=0.95, orient="horizontal", variable=self.opacity,
                  command=lambda *_: self.request_render()).pack(side="left", fill="x", expand=True)

        ttk.Label(tools, text="Zoom:").pack(side="left", padx=(15, 5))
        ttk.Scale(tools, from_=0.25, to=4.0, orient="horizontal", variable=self.zoom,
                  command=lambda *_: self.request_render()).pack(side="left", fill="x", expand=True)

        ttk.Button(tools, text="Fit", command=self.fit_to_window).pack(side="left", padx=8)
        ttk.Button(tools, text="Undo", command=self.undo).pack(side="left", padx=2)
        ...
        self.canvas.bind("<Configure>", lambda _e: self.on_resize())
    ...
    def load_mask(self):
        ...
        self.fit_to_window()
        self.request_render()
        self._update_status(mask_loaded=os.path.basename(p))
    ...
    # -------------------
    def on_resize(self):
        # update fit-scale when the canvas changes size
        if self.img_pil is None:
            return
    ...
    def _compute_fit_scale(self):
        # Fit the image inside the canvas (with small padding)
        cw = max(1, self.canvas.winfo_width() - 20)
        ch = max(1, self.canvas.winfo_height() - 20)
        w, h = self.img_pil.width, self.img_pil.height
        self._fit_scale = min(cw / max(w, 1), ch / max(h, 1), 1.0)

    def _prepare_display_assets(self):
        if self.img_pil is None or self.mask_arr is None:
            return

        scale = float(self._fit_scale) * float(self.zoom.get())
    ...
    def request_render(self):
        # throttle re-rendering so brush stays responsive
        if self._render_job is not None:
            try:
                self.after_cancel(self._render_job)
            except Exception:
                pass
        self._render_job = self.after(10, self.render)
    ...
    def render(self):
        ...
        self._prepare_display_assets()
        if self.disp_img is None or self.disp_mask is None:
            self.canvas.delete("all")
            return
        ...
        # Edge overlay (green outline, full alpha)
        mm = (m > 0).astype(np.uint8)
        # 4-neighborhood edge: mask pixel with at least one neighbor background
        up = np.pad(mm[1:, :], ((0, 1), (0, 0)), mode="constant")
        dn = np.pad(mm[:-1, :], ((1, 0), (0, 0)), mode="constant")
        lf = np.pad(mm[:, 1:], ((0, 0), (0, 1)), mode="constant")
        rt = np.pad(mm[:, :-1], ((0, 0), (1, 0)), mode="constant")
        edge = (mm == 1) & ((up + dn + lf + rt) < 4)
        overlay[edge, 0] = 0
        overlay[edge, 1] = 255
        overlay[edge, 2] = 0
        overlay[edge, 3] = 255
        ...
        self.canvas.create_image(self.canvas_offset_x, self.canvas_offset_y, anchor="nw", image=self.tk_overlay, tags=("img",))
        self._draw_cursor()  # redraw cursor on top
    ...
    def _draw_cursor(self):
        # draw brush circle where the mouse is (for visibility)
        self.canvas.delete("cursor")
        if self.img_pil is None or self.disp_img is None:
            return
        ...
        # convert to displayed image coords to ensure we only show cursor over image
        ...
        self.canvas.create_oval(x - r, y - r, x + r, y + r, outline="#00ff00", width=2, tags=("cursor",))

    def _apply_brush(self, x, y, from_xy=None):
        if self.disp_img is None or self.img_pil is None or self.mask_arr is None:
            return

        # canvas coords -> displayed image coords
        x_img = x - self.canvas_offset_x
        y_img = y - self.canvas_offset_y
    ...
    # Status
    # -------------------
    def _update_status(self, mask_loaded=None):
        if self.img_pil is None or self.mask_arr is None:
            self.status.set("Idle")
            return
        total = int(self.mask_arr.size)
        on = int(np.count_nonzero(self.mask_arr))
        pct = (100.0 * on / total) if total else 0.0
        img_name = os.path.basename(self.img_path.get().strip()) if self.img_path.get().strip() else "(image)"
        parts = [f"Image: {img_name}", f"Mask pixels: {on} ({pct:.2f}%)"]
        if mask_loaded:
            parts.append(f"Loaded mask: {mask_loaded}")
...
def main():
    ...
    # Start maximized (full-screen workspace)
    try:
    ...
    # Optional true fullscreen toggle (F11)
    _fs = {"on": False}
